...
R3	clrf	portb
R3m	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no49
	movfw	3ah		;mantiene el color adentro (verde)
...
no49	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no50
	movfw	3bh		;mantiene el color adentro (amarillo)
...
no50	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no51
	movfw	3ch		;mantiene el color adentro (azul)
...
no51	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no52
	movfw	3dh		;mantiene el color adentro (rojo)
...
no52	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	R3m
	movfw	3eh		;mantiene el color adentro (blanco)
...
R4	;parte aleatoria para la proxima fase con desconteo de 49h--------------
	clrf	portb
RAm	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no64
	movfw	3ah		;mantiene el color adentro (verde)
...
no64	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no65
	movfw	3bh		;mantiene el color adentro (amarillo)
...
no65	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no66
	movfw	3ch		;mantiene el color adentro (azul)
...
no66	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no67
	movfw	3dh		;mantiene el color adentro (rojo)
...
no67	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
...
bien4	;Empiesa la quinta fase del simon say----------------------------------
	clrf	portb		;Esta demas por las dudas
	bsf	portb,0		;Muestre el Ra0 (ej: Verde)
	call	demorav		;Demora variable
	bcf	portb,0		;Apaga el Ra0
	call	demora2
	...
	clrf   PORTB      ;TODO EN 0
	;Empiesa las preguntas-------------------------------------------------
j5	btfss   porta,0     	;aprete el puerto A,0?
	goto   no700     	;si es no me voy a preguntar por los otros puertos
	movlw   .1      	;si es si
	movwf   40h      	;variable para restar despues
	bsf   portb,0      	;Prende el color del puertob,0         
	;Primera pregunta de la funcion aleatoria-------------------------------
        call   demora1      	;Anti rebote de 50mseg
w21	btfsc   porta,0      	;primera pregunta aleatoria
	goto	no71
	goto	R5		;es la parte de la funcion aleatoria
no71	movlw	.1		
	addwf	49h,f		;Se le suma .1 al 49h que es una memoria
	goto	w21

no700	;Pregunta por el otro--------------------------------------------------
	btfss   porta,1     	;aprete el puerto A,1?
	goto   no72     	;si es no me voy a preguntar por los otros puertos
	movlw   .2      	;si es si
	movwf   40h      	;variable para restar despues
	bsf   portb,1      	;Prende el color del puertob,1         
	;Primera pregunta de la funcion aleatoria-------------------------------
        call   demora1      	;Anti rebote de 50mseg
w22	btfsc   porta,1      	;primera pregunta aleatoria
	goto	no73
	goto	R5		;es la parte de la funcion aleatoria
no73	movlw	.1		
	addwf	49h,f		;Se le suma .1 al 49h que es una memoria
...
no72	btfss   porta,2     	;aprete el puerto A,2?
	goto   no74     	;si es no me voy a preguntar por los otros puertos
	movlw   .3      	;si es si
	movwf   40h      	;variable para restar despues
	bsf   portb,2      	;Prende el color del puertob,2         
	;Primera pregunta de la funcion aleatoria-------------------------------
        call   demora1      	;Anti rebote de 50mseg
w23	btfsc   porta,2      	;primera pregunta aleatoria
	goto	no75
	goto	R5		;es la parte de la funcion aleatoria
no75	movlw	.1		
	addwf	49h,f		;Se le suma .1 al 49h que es una memoria
...
no74	btfss   porta,3     	;aprete el puerto A,3?
	goto   no76     	;si es no me voy a preguntar por los otros puertos
	movlw   .4      	;si es si
	movwf   40h      	;variable para restar despues
	bsf   portb,3      	;Prende el color del puertob,3         
	;Primera pregunta de la funcion aleatoria-------------------------------
        call   demora1      	;Anti rebote de 50mseg
w24	btfsc   porta,3      	;primera pregunta aleatoria
	goto	no77
	goto	R5		;es la parte de la funcion aleatoria
no77	movlw	.1		
	addwf	49h,f		;Se le suma .1 al 49h que es una memoria
...
no76	btfss   porta,4     	;aprete el puerto A,4?
	goto  	j5     		;si es no me voy a preguntar por los otros puertos
	movlw   .5      	;si es si
	movwf   40h      	;variable para restar despues
	bsf   portb,4      	;Prende el color del puertob,4         
	;Primera pregunta de la funcion aleatoria-------------------------------
        call   demora1      	;Anti rebote de 50mseg
w25	btfsc   porta,4      	;primera pregunta aleatoria
	goto	no78
	goto	R5		;es la parte de la funcion aleatoria
no78	movlw	.1		
	addwf	49h,f		;Se le suma .1 al 49h que es una memoria
...
R5	;parte aleatoria para la proxima fase con desconteo de 49h--------------
	clrf	portb
Rem1	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no79
	movfw	3ah		;mantiene el color adentro (verde)
	movwf	1fh		;lo usara despues para mostrarlo en la segunda fase
	movlw	.1		;el .1 equivale al color
	movwf	4fh		;se guardara en 4bh para luego en la resta se dira si fue correcto o no
	goto	P5		;funcion de saber si se preciono bien o no

no79	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no80
	movfw	3bh		;mantiene el color adentro (amarillo)
...
no80	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no81
	movfw	3ch		;mantiene el color adentro (azul)
...
no81	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	no82
	movfw	3dh		;mantiene el color adentro (rojo)
...
no82	movlw	.15		;Repite la operacion hasta dar cero
	subwf	49h,f
	btfss	status,2	;pregunta si la cuenta dio cero
	goto	Rem1		;Vuelve para arriba
	movfw	3eh		;mantiene el color adentro (blanco)
	movwf	1fh		;lo usara despues para mostrarlo en la segunda fase
	movlw	.5		;el .1 equivale al color
	movwf	4fh		;se guardara en 4bh para luego en la resta se dira si fue correcto o no
...
no83	movlw	.1
	subwf	0CH,f
	btfss	status,2	;Z = 1 osea que si dio cero la resta
	goto	no84		;Significa que solo dio una vuelta y pregunta por el primero
	movfw	40h
	subwf	4ch,w		;al ser ,w mantiene el valor que adquirio paras otras fases
	btfss	status,2	;Z = 1 osea que si dio cero la resta
	goto	perdi
...
no84	movlw	.1
	subwf	10h,f
	btfss	status,2	;Z = 1 osea que si dio cero la resta
	goto	no85	 	;Significa que solo dio una vuelta y pregunta por el primero
	movfw	40h		;si es si
	subwf	4bh,w		;al ser ,w mantiene el valor que adquirio paras otras fases
	btfss	status,2	;Z = 1 osea que si dio cero la resta
	goto	perdi
...
no85	movfw	40h		;si es si
	subwf	4ah,w		;al ser ,w mantiene el valor que adquirido para otras fases
	btfss	status,2	;Z = 1 osea que si dio cero la resta
	goto	perdi
	call	demora2
	goto	j5		;Vuelve para la segunda vuelta
